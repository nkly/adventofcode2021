...
main =
  ...
  where
    solve [xarea@(_, x2), yarea@(y1, _)] =
      length $ catMaybes [simulate xarea yarea (xvel, yvel) | xvel <- [0 .. x2 + 1], yvel <- [y1 .. 500]]
    solve _ =
      error "invalid input"
    simulate (x1, x2) (y1, y2) (xvel1, yvel1)
      | x1 <= lx && lx <= x2 && y1 <= ly && ly <= y2 =
        Just (maximum $ snd <$> path)
      | otherwise =
        Nothing
      where
        (lx, ly) =
          last path
        path =
          unfoldr (uncurry step) ((0, 0), (xvel1, yvel1))
        step (x, y) (xvel, yvel)
          | y < y1 || x > x2 =
            Nothing
          | otherwise =
            Just ((x, y), ((x + xvel, y + yvel), (xvel - dx, yvel - 1)))
          where
            dx = max (-1) (min 1 xvel)
    parse =
      fmap (bimap readInt (readInt . tail) . break' (== '.') . snd . break' (== '=')) . drop 2 . words
    break' p =
      second tail . break p
    readInt s
      | last s == ',' =
