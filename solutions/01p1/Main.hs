...
main :: IO ()
main =
  getContents >>= print . snd . foldl detectChange (Nothing, 0) . fmap (read :: String -> Int) . lines
  where
    detectChange (Nothing, _) depth =
      (Just depth, 0)
    detectChange (Just prevDepth, numIncreases) depth
      | prevDepth < depth = (Just depth, succ numIncreases)
      | otherwise = (Just depth, numIncreases)
