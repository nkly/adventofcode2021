...
main :: IO ()
main =
  getContents >>= (V.thaw . V.fromList . fmap digitToInt . concat . lines) >>= solve 100 >>= print
  where
    solve maxSteps world = do
      sum <$> for [1 .. maxSteps] simulate
      where
        simulate _ = do
          ...
          pure nFlashes
        flashAll nFlashes = do
          n <- M.ifoldM' flashOnce 0 world
          if n > 0 then flashAll (nFlashes + n) else pure nFlashes
        flashOnce n pos lvl
          | lvl <= 9 =
            pure n
          | otherwise = do
