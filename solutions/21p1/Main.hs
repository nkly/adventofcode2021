...
main :: IO ()
main =
  getContents >>= print . solve 0 1 (0, 0) . parse
  where
    solve turn dice (s1, s2) (p1, p2)
      | s1 >= 1000 || s2 >= 1000 =
        3 * turn * min s1 s2
      | even turn =
        solve (turn + 1) (clampDice (dice + 3)) (s1 + p1', s2) (p1', p2)
      | otherwise =
        solve (turn + 1) (clampDice (dice + 3)) (s1, s2 + p2') (p1, p2')
      where
        dp =
          clampDice dice + clampDice (dice + 1) + clampDice (dice + 2)
        p1' =
          clampPos (p1 + dp)
        p2' =
          clampPos (p2 + dp)
        ...
        clampDice d =
          if d `mod` 100 == 0 then 100 else d `mod` 100
    parse =
      bisequence
