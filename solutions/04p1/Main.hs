...
main =
  getContents
    >>= print
      ...
      . lines
  where
    solve prevDraw (numbers, boards) =
      case find isWinning boards of
        Just winner ->
          prevDraw * foldl (\acc (n, s) -> if s then acc else n + acc) 0 (concat winner)
        Nothing ->
          solve (head numbers) (tail numbers, boards')
      where
        isWinning =
          bior . bisequence (any (all snd), any (all snd) . transpose)
        boards' =
          fmap (fmap (\(n, s) -> if n == head numbers then (n, True) else (n, s))) <$> boards
    splitOn n =
