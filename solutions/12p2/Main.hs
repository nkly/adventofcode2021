...
main :: IO ()
main =
  getContents >>= print . length . findPaths . toGraph . fmap (second tail . break (== '-')) . lines
  where
    toGraph =
      foldl (\g (f, t) -> M.insertWith (<>) f [t] $ M.insertWith (<>) t [f] g) M.empty
    findPaths =
      S.fromList . fmap concat . go [] True "start"
      where
        go visited canVisitTwice v g
          | v == "end" =
            [["end"]]
          | v == "start" =
            visit [v] True
          | canVisitTwice =
            visit visited' True <> visit visited False
          | otherwise =
            visit visited' False
          where
            toVisit =
              filter (`notElem` visited) $ M.findWithDefault [] v g
            visited' =
              if isLower $ head v then v : visited else visited
            visit vs cvt =
              concatMap (\v' -> (v :) <$> go vs cvt v' g) toVisit
