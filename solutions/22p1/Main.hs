...
main :: IO ()
main =
  getContents >>= print . solve S.empty . parse
  where
    solve cubesOn [] =
      length cubesOn
    solve cubesOn ((state, (xa@(xs, xe), ya@(ys, ye), za@(zs, ze))) : ss)
      | outOfRange xa || outOfRange ya || outOfRange za =
        solve cubesOn ss
      | state == "on" =
        solve (foldl' (flip S.insert) cubesOn cubesInArea) ss
      | otherwise =
        solve (foldl' (flip S.delete) cubesOn cubesInArea) ss
      where
        cubesInArea = [(x, y, z) | x <- [xs .. xe], y <- [ys .. ye], z <- [zs .. ze]]
        outOfRange (s, e) = s < -50 || s > 50 || e < -50 || e > 50
    parse =
      fmap (second parseCoords . break' (== ' ')) . lines
      where
        parseCoords s =
          case splitOn ',' s of
            [xs, ys, zs] ->
              (parseCoord xs, parseCoord ys, parseCoord zs)
            _ ->
              error $ "invalid coords: '" <> s <> "'"
        parseCoord =
          bimap (read :: String -> Int) (read . tail :: String -> Int) . break' (== '.') . drop 2
        break' p =
          second tail . break p
        splitOn n =
