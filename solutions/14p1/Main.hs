...
main :: IO ()
main =
  getContents >>= print . count . uncurry (iter 10) . parse
  where
    count =
      bisum . bisequence (maximum, negate . minimum) . fmap length . group . sort
    iter step polymer rules
      ...
      where
        matchPair p =
          head p : fromMaybe "" (lookup p rules)
        polymer' =
          concat (matchPair . biList <$> zip polymer (tail polymer)) ++ [last polymer]
    parse =
      bisequence (head, fmap (bisequence (take 2, drop 6)) . drop 2) . lines
