...
main :: IO ()
main =
  getContents >>= print . solve . M.fromLists . fmap (fmap digitToInt) . lines
  where
    solve matrix =
      (sum . fmap snd . filter fst . M.toList) $
        M.ifor matrix $ \(row, col) x ->
          (all (>= x) $ neighbors row col, x + 1)
      where
        rclamp =
          max 0 . min (M.nrows matrix - 1)
        cclamp =
          max 0 . min (M.ncols matrix - 1)
        neighbors row col =
          M.toList $ M.submatrix (rclamp $ pred row, cclamp $ pred col) (rclamp $ succ row, cclamp $ succ col) matrix
