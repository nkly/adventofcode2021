...
main :: IO ()
main =
  getContents >>= print . product . sequence [findNum fst 0, findNum snd 0] . lines
  where
    findNum _ _ [] =
      error "shouldn't happen"
    findNum _ _ [match] =
      foldl (\acc c -> acc * 2 + digitToInt c) 0 match
    findNum selector pos input =
      findNum selector (succ pos) $ filter (\s -> s !! pos == selector (bitsAt pos input)) input
    bitsAt pos =
      (\(numZeroes, numOnes) -> if numZeroes <= numOnes then ('0', '1') else ('1', '0'))
        . foldl countBits (0, 0)
        . (!! pos)
        . transpose
    countBits acc c
