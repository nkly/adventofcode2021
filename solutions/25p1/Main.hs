...
data Location = East | South | Empty deriving (Eq)
...
main =
  ...
  where
    ...
    play count field
      ...
      where
        field' =
          moveSouth $ moveEast field
        moveEast f =
          foldl' (\f' pos -> M.swap pos (nextEast pos) f') f $ filter (canMoveEast f) [(r, c) | r <- [0 .. M.nrows f - 1], c <- [0 .. M.ncols f - 1]]
        moveSouth f =
          foldl' (\f' pos -> M.swap pos (nextSouth pos) f') f $ filter (canMoveSouth f) [(r, c) | r <- [0 .. M.nrows f - 1], c <- [0 .. M.ncols f - 1]]
        canMoveEast f pos =
          M.unsafeGet pos f == East && M.unsafeGet (nextEast pos) f == Empty
        canMoveSouth f pos =
          M.unsafeGet pos f == South && M.unsafeGet (nextSouth pos) f == Empty
        nextEast (r, c) =
          (r, (c + 1) `mod` M.ncols field)
        nextSouth (r, c) =
          ((r + 1) `mod` M.nrows field, c)
    parse =
      M.fromLists . fmap (fmap toSeaCucumber) . lines
      where
        toSeaCucumber c
          | c == '.' = Empty
          | c == '>' = East
          | c == 'v' = South
          | otherwise = error $ "invalid input: " <> [c]
